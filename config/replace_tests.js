            ...
            if (testFilePath.includes("*")) {
                ...
                glob(testFilePath, options, function (er, currentFilePaths) {
                    for (const path of currentFilePaths) {
                        ...
                        fs.readdir(`./${TESTS_FOLDER}/99x-SE-DotNet-2022-dotnet/HungryNowTest/`, (err, originalFiles) => {
...
const copyFile = (srcFileName, destPath) => {
    fs.copyFile(`${TESTS_FOLDER}/99x-SE-DotNet-2022-dotnet/HungryNowTest/${srcFileName}`, `../${destPath}`, (err) => {
        if (err) throw err;
    });
    return "Copied";
}
